...
Core features include domain registration with a 2-step commit-reveal process, address resolution, payment forwarding, reverse resolution, domain extension/renewal, and ERC-721 NFT ownership. Security features include front-running protection, reentrancy protection, a 30-day grace period for expired domains (set in BaseRegistrar), and on-chain primary domain storage. Pricing is tiered based on domain character length (3 chars: 100 TRUST/year, 4 chars: 70 TRUST/year, 5+ chars: 30 TRUST/year). A backend-driven metadata system ensures accurate and persistent NFT metadata via dynamic SVG image generation and a comprehensive metadata API.
