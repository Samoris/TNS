...
const V2_CONTROLLER = "0x8A5F965e8D5e6330f99B81674670aC6f643F1A8C";

const DOMAINS_TO_MIGRATE = ["sourav", "mooneth"];

async function main() {
  const [deployer] = await ethers.getSigners();
  console.log("Migrating V2 stragglers with:", deployer.address);
  ...
  console.log("2. Migrating domains...");
  let migrated = 0;
  ...
  for (const name of DOMAINS_TO_MIGRATE) {
    ...
    try {
      const existingOwner = await v3Registrar.ownerOf(tokenId);
      console.log(`  [SKIP] ${name}.trust already on V3, owner: ${existingOwner}`);
      continue;
    } catch {}
  ...
  console.log("\n4. Disabling V2 controller on V2 BaseRegistrar...");
  const isV2Active = await v2Registrar.controllers(V2_CONTROLLER);
  if (isV2Active) {
    const txDisable = await v2Registrar.removeController(V2_CONTROLLER);
    await txDisable.wait();
    console.log("V2 controller disabled.");
    const isV2StillActive = await v2Registrar.controllers(V2_CONTROLLER);
    console.log("V2 controller still active:", isV2StillActive);
  } else {
    console.log("V2 controller already disabled.");
  }
  ...
  console.log("V2 controller disabled: yes");
  console.log("Balance:", ethers.formatEther(await ethers.provider.getBalance(deployer.address)), "TRUST");
}
