...
/**
 * @title TNS Registry - With ERC721 Functions
 * @dev Gas-efficient domain registration contract for .trust domains with ERC721 compatibility
 * @notice Simplified ERC721 implementation with totalSupply and nextTokenId functions
 */
contract TNSRegistryWithERC721 {
    ...
    /**
     * @dev ERC721 - Get total number of tokens minted
    ...
    /**
     * @dev ERC721 - Get owner of a specific token ID
     * @param tokenId The token ID to query
     * @return Owner address of the token
     */
    function ownerOf(uint256 tokenId) public view returns (address) {
        string memory domain = tokenIdToDomain[tokenId];
        require(bytes(domain).length > 0, "Token does not exist");
    ...
    /**
     * @dev ERC721 - Get the balance of tokens owned by an address
     * @param tokenOwner Address to query
     * @return Number of active tokens owned
     */
    function balanceOf(address tokenOwner) public view returns (uint256) {
        require(tokenOwner != address(0), "Zero address not allowed");
        
        string[] memory userDomains = ownerDomains[tokenOwner];
        uint256 activeCount = 0;
        
        for (uint256 i = 0; i < userDomains.length; i++) {
            if (domains[userDomains[i]].exists && !isExpired(userDomains[i])) {
                activeCount++;
            }
        }
    ...
    /**
     * @dev Get token URI (returns domain name as identifier)
     * @param tokenId The token ID to query
     * @return Domain name associated with the token
     */
