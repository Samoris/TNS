...
contract TNSRegistryERC721 is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Batch migrate domains from old contract (admin only, no payment required)
     * @param domainNames Array of domain names (without .trust suffix)
     * @param owners Array of domain owners (must match domainNames length)
     * @param expirationTimes Array of expiration timestamps (must match domainNames length)
     * @param oldTokenIds Array of token IDs from old contract (for reference)
     * @notice This function bypasses normal registration to migrate existing domains
     */
    function batchMigrateDomains(
        ...
        require(
            ...
            owners.length == expirationTimes.length &&
            expirationTimes.length == oldTokenIds.length,
            "Array lengths must match"
        );
        
        for (uint256 i = 0; i < domainNames.length; i++) {
            string calldata domain = domainNames[i];
            address domainOwner = owners[i];
            uint256 expirationTime = expirationTimes[i];
            
            // Validate domain
            require(bytes(domain).length >= 3 && bytes(domain).length <= 63, "Invalid domain length");
            require(!domains[domain].exists, "Domain already exists");
            require(domainOwner != address(0), "Invalid owner address");
            ...
            ownerDomains[domainOwner].push(domain);
            
            // Mint the ERC-721 NFT to the original owner
            _safeMint(domainOwner, newTokenId);
            
            // Emit registration event
            emit DomainRegistered(domain, domainOwner, newTokenId, expirationTime);
        }
    }
    
    /**
     * @dev Set primary domains after migration (admin only)
     * @param owners Array of owner addresses
     * @param domainNames Array of primary domain names (must match owners length)
     */
    function batchSetPrimaryDomains(
        address[] calldata owners,
        string[] calldata domainNames
    ) external onlyOwner {
        require(owners.length == domainNames.length, "Array lengths must match");
        
        for (uint256 i = 0; i < owners.length; i++) {
            address domainOwner = owners[i];
            string calldata domain = domainNames[i];
            
            // Verify the domain exists and is owned by the address
            if (bytes(domain).length > 0 && domains[domain].exists) {
                uint256 tokenId = domainToTokenId[domain];
                if (ownerOf(tokenId) == domainOwner) {
                    primaryDomain[domainOwner] = domain;
                    emit PrimaryDomainSet(domainOwner, domain);
                }
            }
        }
    }
