...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const linkingChallenges = new Map<string, { nonce: string; primaryAddress: string; socialAddress: string; expiresAt: number }>();

  app.post("/api/linked-accounts/challenge", async (req, res) => {
    try {
      const { primaryAddress, socialAddress } = req.body;
      if (!primaryAddress || !socialAddress) {
        return res.status(400).json({ error: "primaryAddress and socialAddress are required" });
      }

      const existing = await storage.getLinkedAccountBySocial(socialAddress);
      if (existing) {
        return res.status(409).json({ error: "This social account is already linked to a wallet" });
      }

      const nonce = createHash('sha256').update(`${primaryAddress}${socialAddress}${Date.now()}${Math.random()}`).digest('hex').slice(0, 32);
      const expiresAt = Date.now() + 5 * 60 * 1000;

      linkingChallenges.set(nonce, { nonce, primaryAddress: primaryAddress.toLowerCase(), socialAddress: socialAddress.toLowerCase(), expiresAt });

      const message = `Link Social Account to TNS\n\nI authorize linking my social login wallet to my primary wallet for Trust Name Service.\n\nPrimary Wallet: ${primaryAddress.toLowerCase()}\nSocial Wallet: ${socialAddress.toLowerCase()}\nNonce: ${nonce}`;
  ...
  app.post("/api/linked-accounts/verify-link", async (req, res) => {
    try {
      const { nonce, primarySignature, socialSignature, socialProvider, socialEmail, socialName } = req.body;
      if (!nonce || !primarySignature || !socialSignature) {
        return res.status(400).json({ error: "nonce, primarySignature, and socialSignature are required" });
      }

      const challenge = linkingChallenges.get(nonce);
      if (!challenge) {
        return res.status(400).json({ error: "Invalid or expired challenge" });
      }

      if (Date.now() > challenge.expiresAt) {
        linkingChallenges.delete(nonce);
        return res.status(400).json({ error: "Challenge expired" });
      }

      const message = `Link Social Account to TNS\n\nI authorize linking my social login wallet to my primary wallet for Trust Name Service.\n\nPrimary Wallet: ${challenge.primaryAddress}\nSocial Wallet: ${challenge.socialAddress}\nNonce: ${nonce}`;

      const recoveredPrimary = ethers.verifyMessage(message, primarySignature).toLowerCase();
      const recoveredSocial = ethers.verifyMessage(message, socialSignature).toLowerCase();

      if (recoveredPrimary !== challenge.primaryAddress) {
        return res.status(401).json({ error: "Primary wallet signature verification failed" });
      }
      if (recoveredSocial !== challenge.socialAddress) {
        return res.status(401).json({ error: "Social wallet signature verification failed" });
      }

      linkingChallenges.delete(nonce);

      const link = await storage.createLinkedAccount({
        primaryAddress: challenge.primaryAddress,
        socialAddress: challenge.socialAddress,
        socialProvider: socialProvider || "web3auth",
      ...
      res.status(500).json({ error: "Failed to verify and create link" });
    }
  });

  app.get("/api/linked-accounts/resolve/:address", async (req, res) => {
    try {
      const { address } = req.params;
      const link = await storage.getLinkedAccountBySocial(address);
      if (link) {
        return res.json({ primaryAddress: link.primaryAddress, linked: true });
      }
      res.json({ primaryAddress: address.toLowerCase(), linked: false });
    } catch (error) {
  ...
  app.get("/api/linked-accounts/by-primary/:address", async (req, res) => {
    try {
      const { address } = req.params;
      const links = await storage.getLinkedAccountsByPrimary(address);
      res.json(links);
    } catch (error) {
  ...
  const unlinkChallenges = new Map<string, { nonce: string; primaryAddress: string; socialAddress: string; expiresAt: number }>();

  app.post("/api/linked-accounts/unlink-challenge", async (req, res) => {
    try {
      const { primaryAddress, socialAddress } = req.body;
      if (!primaryAddress || !socialAddress) {
        return res.status(400).json({ error: "primaryAddress and socialAddress are required" });
      ...
      const link = await storage.getLinkedAccountBySocial(socialAddress);
      if (!link) {
        return res.status(404).json({ error: "Linked account not found" });
      }
      if (link.primaryAddress !== primaryAddress.toLowerCase()) {
        return res.status(403).json({ error: "Only the primary wallet owner can unlink accounts" });
      }

      const nonce = createHash('sha256').update(`unlink-${primaryAddress}${socialAddress}${Date.now()}${Math.random()}`).digest('hex').slice(0, 32);
      const expiresAt = Date.now() + 5 * 60 * 1000;
      unlinkChallenges.set(nonce, { nonce, primaryAddress: primaryAddress.toLowerCase(), socialAddress: socialAddress.toLowerCase(), expiresAt });

      const message = `Unlink Social Account from TNS\n\nI authorize removing the link between my social login wallet and my primary wallet.\n\nPrimary Wallet: ${primaryAddress.toLowerCase()}\nSocial Wallet: ${socialAddress.toLowerCase()}\nNonce: ${nonce}`;
      res.json({ nonce, message });
    } catch (error) {
      console.error("Unlink challenge error:", error);
      res.status(500).json({ error: "Failed to generate unlink challenge" });
    }
  });

  app.delete("/api/linked-accounts/:socialAddress", async (req, res) => {
    try {
      const { socialAddress } = req.params;
      const { signature, primaryAddress, nonce } = req.body;

      if (!signature || !primaryAddress || !nonce) {
        return res.status(400).json({ error: "signature, primaryAddress, and nonce are required" });
      }

      const challenge = unlinkChallenges.get(nonce);
      if (!challenge) {
        return res.status(400).json({ error: "Invalid or expired unlink challenge" });
      }
      if (Date.now() > challenge.expiresAt) {
        unlinkChallenges.delete(nonce);
        return res.status(400).json({ error: "Unlink challenge expired" });
      }
      if (challenge.socialAddress !== socialAddress.toLowerCase() || challenge.primaryAddress !== primaryAddress.toLowerCase()) {
        return res.status(400).json({ error: "Challenge does not match the request" });
      }

      const message = `Unlink Social Account from TNS\n\nI authorize removing the link between my social login wallet and my primary wallet.\n\nPrimary Wallet: ${challenge.primaryAddress}\nSocial Wallet: ${challenge.socialAddress}\nNonce: ${nonce}`;
      const recovered = ethers.verifyMessage(message, signature).toLowerCase();

      if (recovered !== challenge.primaryAddress) {
        return res.status(401).json({ error: "Signature verification failed" });
      }

      unlinkChallenges.delete(nonce);
      await storage.deleteLinkedAccount(socialAddress);
      res.json({ success: true });
    } catch (error) {
